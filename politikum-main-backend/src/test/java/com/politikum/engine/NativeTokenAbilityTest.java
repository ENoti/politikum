...
class NativeTokenAbilityTest {
    @Test void tokenAbilitiesMatchLegacy() throws Exception {
        try (Context guest = Context.newBuilder("js").build()) {
            ...
            String actual;
            try (var in = new ClassPathResource("engine/token-ability-scenarios.js").getInputStream()) {
                actual = guest.eval("js", new String(in.readAllBytes(), StandardCharsets.UTF_8)).asString();
            }
            try (var in = new ClassPathResource("engine/token-ability-legacy.json").getInputStream()) {
