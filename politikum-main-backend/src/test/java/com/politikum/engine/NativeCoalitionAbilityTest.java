...
class NativeCoalitionAbilityTest {
    @Test void coalitionAbilitiesMatchLegacy() throws Exception {
        try (Context guest = Context.newBuilder("js").build()) {
            ...
            String actual;
            try (var in = new ClassPathResource("engine/coalition-ability-scenarios.js").getInputStream()) {
                actual = guest.eval("js", new String(in.readAllBytes(), StandardCharsets.UTF_8)).asString();
            }
            try (var in = new ClassPathResource("engine/coalition-ability-legacy.json").getInputStream()) {
