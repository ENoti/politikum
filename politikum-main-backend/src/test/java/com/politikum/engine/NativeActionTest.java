...
class NativeActionTest {
    @Test void actionsMatchLegacy() throws Exception {
        try(Context guest=Context.newBuilder("js").build()) {
            ...
            String actual=guest.eval("js",read("engine/action-scenarios.js")).asString();
            var expected=JsonUtils.mapper().readTree(read("engine/action-legacy.json"));
