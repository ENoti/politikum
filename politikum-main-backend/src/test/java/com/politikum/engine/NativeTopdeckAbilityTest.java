...
class NativeTopdeckAbilityTest {
    @Test void topdeckAbilityMatchesLegacy() throws Exception {
        try (Context guest = Context.newBuilder("js").build()) {
            ...
            String actual;
            try (var in = new ClassPathResource("engine/topdeck-ability-scenarios.js").getInputStream()) {
                actual = guest.eval("js", new String(in.readAllBytes(), StandardCharsets.UTF_8)).asString();
            }
            try (var in = new ClassPathResource("engine/topdeck-ability-legacy.json").getInputStream()) {
